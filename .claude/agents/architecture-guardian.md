...
Current findings are tracked in `docs/generated/architecture/known-violations.md`
(regenerated by `architecture-doc-agent` from this agent's live review — this file
no longer hand-embeds the table). Run a fresh review and hand the findings to
`architecture-doc-agent` to keep it current.
